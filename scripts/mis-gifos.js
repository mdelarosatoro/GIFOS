...
function decideFavoriteDOMContent() {
    if (JSON.parse(localStorage.getItem("mis-gifos")).length > 0) {
        addFavorites();
    } else {
...
function addStaticDOMFavorites() {
    ...
    favoriteText.textContent = 'Anímate a crear tu primer GIFO!';
...
async function addFavorites() {
    try {
        ...
        const arrayFavorites = JSON.parse(localStorage.getItem("mis-gifos"));
...
function createSearchElements(favoritesToAdd) {
    ...
    if (JSON.parse(localStorage.getItem("mis-gifos")).length <= 12) {
        let favoritesGrid = buildFavoritesGrid(favoritesToAdd);

        favoritesContainerDOM.appendChild(favoritesGrid);

    } else if (JSON.parse(localStorage.getItem("mis-gifos")).length > 12) {
